...
public class Recognition implements Cloneable {
	...
	// 判定为陌生人的阈值
	private float unknowThreshold = 0.6f;//相似度大于0.6为陌生人，人脸重构效果越好阈值可以相对的减小，加大要求
	...
	public CvMat reconstructFace(CvMat preprocessedFace) {
		try {
			...
			CvMat eigenvectors = recognizer.getMat("eigenvectors");  //此处有问题
	...
	public int recognise(CvMat preprocessedFace) {
		...
		System.out.println("recognise验证识别可靠性(重构 vs 预处理图像<0.6)："+similarity);

		int identity = -1;
		if (similarity < unknowThreshold) {
			identity = recognizer.predict(preprocessedFace);//识别，并返回人脸编号（即对应的学号）
		}
		return identity; //返回人脸编号
	}
